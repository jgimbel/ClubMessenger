...
class GoogleVoice:
    ...
    def send(self, number, text):
        """
        Sends an SMS txt message.
        Parameters:
            <number>: number to send to
            <text>: contents of txt message to send

        Returns: True|False
        """
        ret = self.get_auth_url(SENDSMS_URL,
            {'phoneNumber': number, 'text': text, '_rnr_se': self.rnr_se, 'id': ''},
            mode='raw')
